...
class Demodulation(object):
    ...
    def set_parameters(self, config={}):
        ...
        self.iq_ratio = np.zeros(self.number_readout_waveforms)
        self.iq_skew = np.zeros(self.number_readout_waveforms)
        for w in range(self.number_readout_waveforms):
            self.iq_ratio[w] = config.get('Readout I/Q ratio %d' % (w + 1))
            #convert iq_skew from degrees to radians
            self.iq_skew[w] = (np.pi/180)*config.get('Readout IQ skew %d' % (w + 1))
