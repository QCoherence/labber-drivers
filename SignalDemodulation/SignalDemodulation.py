...
class Driver(InstrumentDriver.InstrumentWorker):
    ...
    def performGetValue(self, quant, options={}):
        ...
        if quant.name == 'Value':
            ...
            valueI = np.mean(self.getIQAmplitudes('Input I data'))
        elif quant.name == 'Value - Single shot':
            # calculate I/Q signal here
            valueI = self.getIQAmplitudes('Input I data')
        elif quant.name.startswith('Value #'):
            index = int(quant.name[-1])
            sDemodFreq = 'Mod. frequency #' + str(index)
            dFreq = self.getValue(sDemodFreq)
            valueI = np.mean(self.getIQAmplitudes_MultiFreq(dFreq,'Input I data'))
        else:
            # just return the quantity value
            valueI = quant.getValue()

        self.perform_IQdemodulation = self.getValue("Use IQ demodulation")
        if self.perform_IQdemodulation:
            if quant.name == 'Value':
                # calculate I/Q signal here
                valueQ = np.mean(self.getIQAmplitudes('Input Q data'))
            elif quant.name == 'Value - Single shot':
                # calculate I/Q signal here
                valueQ = self.getIQAmplitudes('Input Q data')
            elif quant.name.startswith('Value #'):
                index = int(quant.name[-1])
                sDemodFreq = 'Mod. frequency #' + str(index)
                dFreq = self.getValue(sDemodFreq)
                valueQ = np.mean(self.getIQAmplitudes_MultiFreq(dFreq,'Input Q data'))
            else:
                # just return the quantity value
                valueQ = quant.getValue()
    ...
    def getIQAmplitudes_MultiFreq(self,dFreq,string):
        """Calculate complex signal from data and reference"""
        # get parameters
        skipStart = self.getValue('Skip start')
        nSegment = int(self.getValue('Number of segments'))
