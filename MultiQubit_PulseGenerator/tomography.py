...
class ProcessTomography(object):
    ...
    def set_parameters(self, config={}):
        ...
        n_qubit = int(config.get('Number of qubits'))
        tomo_enabled = config.get('Generate process tomography prepulse') or config.get('Generate state tomography postpulse')
        if self.nProcessTomoQubits == 1:
            self.qubit1ID = int(config.get('Qubit for tomography'))
            self.prepulse_index = config.get(
                'Process tomography prepulse index 1-QB')
        elif self.nProcessTomoQubits == 2:
            self.qubit1ID = int(config.get('Qubit 1 for tomography'))
            self.qubit2ID = int(config.get('Qubit 2 for tomography'))
            if self.qubit1ID == self.qubit2ID and tomo_enabled:
                raise Exception('Two-qubit tomography requires two distinct qubits')
            self.prepulse_index = config.get(
                'Process tomography prepulse index 2-QB')
        if any(n > n_qubit for n in [self.qubit1ID, self.qubit2ID]) and tomo_enabled:
            raise Exception('Selected qubits for tomography exceed the number of qubits in the sequence')
    ...
    def set_parameters(self, config={}):
        ...
        if self.tomography_scheme == 'Single qubit':
            ...
            self.tomography_index = config.get('Tomography pulse index 1-QB')
                                        ...
                                        'Two qubit (30 pulse set)',
                                        'Two qubit (36 pulse set)']:
            self.twoQBtomoID1 = int(config.get('Qubit 1 for tomography'))
            self.twoQBtomoID2 = int(config.get('Qubit 2 for tomography'))
